 ...
 let user = message.mentions.members.first() || message.guild.members.cache.get(args[0])
        ...
        if (message.author == owner) { 
            ...
            if(!user) return message.channel.send("Debes mencionar a un usuario vÃ¡lido.")

            if (!args[1]) return message.channel.send("Debes mencionar el rol que deseas remover del usuario.") 

            if (!role) return message.channel.send("Debes mencionar un rol vÃ¡lido.") 
        ...
        if (message.member.roles.highest.comparePositionTo(user.roles.highest) <= 0)  return message.channel.send("No puedes remover roles a ese usuario, puede que tenga un rol igual o superior al tuyo.") 
  ...
  .setDescription(`El rol **${role}** fue removido correctamente del usuario **${user}**.`)
        )   
      }